...
export function avatarStyleIndex(seed: string): number {
  const safeSeed = typeof seed === 'string' && seed.length > 0 ? seed : MARKER_NAME_FALLBACK;
 ...
 */
export function normalizeMarkerName(firstName: string | null | undefined): string {
  const text = typeof firstName === 'string' ? firstName.replace(/\s+/g, ' ').trim() : '';
