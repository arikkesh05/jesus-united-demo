    ...
    .map((item): PulpitKitOutlinePoint | null => {
      ...
      if (isPlainObject(item)) {
        const section = toText(
          item.section ?? item.title ?? item.point ?? item.heading,
        );
        if (section === "") return null;
        return {
          section,
          subtext: toText(
            item.subtext ?? item.detail ?? item.description ?? item.notes,
          ),
        };
      }
...
export function parsePulpitKitRow(raw: unknown): PulpitKit | null {
  ...
  const estimatedMinutes = toFiniteNumber(
    raw.estimated_minutes ?? raw.duration_minutes,
  );
  ...
  const kit: PulpitKit = {
    ...
    discussion_questions: toStringArray(
      raw.discussion_questions ?? raw.questions,
    ),
    key_quote: toOptionalText(raw.key_quote ?? raw.quote),
    call_to_action: toOptionalText(raw.call_to_action ?? raw.application),
    target_sunday: toText(
      raw.target_sunday ?? raw.published_at ?? raw.created_at,
    ),
...
export async function getLatestPulpitKit(): Promise<PulpitKit | null> {
  try {
    ...
    if (error) {
      console.error(
        "Error fetching pulpit kit, using local fallback kit:",
        error.message,
      );
    ...
    console.error(
      "Unexpected error fetching pulpit kit, using local fallback kit:",
      error,
    );
    return FALLBACK_PULPIT_KIT;
  }
