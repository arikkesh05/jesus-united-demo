...
import type {
  ModerateGatheringArgs,
  PrayerVisibilityUpdate,
} from "@/lib/types";
...
export async function moderateGathering(
  ...
  try {
    ...
    const supabase = await requireModerator();
    const args: ModerateGatheringArgs = {
      p_submission_id: submissionId,
      p_action: action,
    };
    const { data, error } = await supabase.rpc("moderate_gathering", args);
    if (error || data !== true)
      throw new Error("Moderation failed or submission already reviewed");
...
export async function setPrayerVisibility(
  ...
  try {
    ...
    if (error || !Array.isArray(data) || data.length !== 1)
      throw new Error("Prayer not updated");
    return { ok: true, error: null };
  } catch {
    return {
      ok: false,
...
export async function deletePrayerRequest(
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .select("id");
    if (error || !Array.isArray(data) || data.length !== 1)
      throw new Error("Prayer not removed");
    return { ok: true, error: null };
  } catch {
    return {
      ok: false,
      error: "The prayer could not be removed. Please try again.",
    };
  }
}
