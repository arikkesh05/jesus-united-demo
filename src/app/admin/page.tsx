  ...
  if (access !== 'allowed') {
    return (
      <main className="mx-auto max-w-3xl px-6 py-16">
        <section className="rounded-3xl border border-sand bg-white p-8 shadow-soft">
          ...
          <div className="mt-6 flex gap-4">
            ...
            {access === 'denied' ? <button type="button" onClick={refresh} className="rounded-full bg-[#C5A880] px-4 py-2 text-sm font-bold focus-visible:ring-2 focus-visible:ring-gold">Retry access</button> : null}
          </div>
        </section>
      </main>
    );
  ...
  return (
    <div className="min-h-screen bg-canvas font-sans text-espresso">
      <main className="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
        <header className="rounded-3xl border border-gold/50 bg-white p-6 shadow-soft sm:p-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link
              href="/"
              className="inline-flex shrink-0 items-center rounded-full border border-sand bg-white px-4 py-2 text-xs font-bold text-espresso transition hover:border-gold hover:bg-pill focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold/50"
            >
              Back to site
            </Link>
          </div>
        ...
        <button type="button" onClick={refresh} disabled={busySubmissionIds.length > 0 || busyPrayerIds.length > 0}
          className="mt-4 rounded-full border border-sand bg-white px-4 py-2 text-sm font-bold focus-visible:ring-2 focus-visible:ring-gold disabled:opacity-50">Refresh queues</button>
            ...
            {notice ? (
              <p
                ...
                className={`mb-4 rounded-2xl border px-4 py-3 text-sm font-medium ${
                  notice.kind === 'success'
                    ? 'border-[#C5A880] bg-pill text-pill-ink'
            ...
            {loadError ? null : submissions === null ? (
              <div role="status" aria-busy="true" className="grid grid-cols-1 gap-6">
                {[0, 1].map((skeleton) => (
                  <div
                    key={skeleton}
                    className="h-44 animate-pulse rounded-3xl border border-sand bg-white"
                  />
                ))}
              </div>
              ...
              <ul className="grid list-none grid-cols-1 gap-6">
                {submissions.map((submission) => {
                  ...
                  return (
                    <li
                      key={submission.id}
                      className="rounded-3xl border border-sand bg-white p-6 shadow-soft"
                    >
                      ...
                      <div className="mt-5 flex flex-wrap items-center gap-2 border-t border-sand pt-4">
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 rounded-full bg-[#C5A880] px-4 py-2 text-xs font-bold text-espresso transition hover:bg-[#B49770] hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold/50 disabled:cursor-not-allowed disabled:opacity-60"
                        >
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 rounded-full border border-sand bg-white px-4 py-2 text-xs font-bold text-red-700 transition hover:border-red-300 hover:bg-red-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold/50 disabled:cursor-not-allowed disabled:opacity-60"
                        >
        ...
        {activeTab === 'prayers' ? (
          <section id="admin-panel-prayers" role="tabpanel" aria-labelledby="admin-tab-prayers" tabIndex={0} className="mt-4">
            ...
            {loadError ? null : prayers === null ? (
              ...
              <p className="rounded-3xl border border-dashed border-sand bg-white p-8 text-center">No prayer requests to review.</p>
            ) : (
              <ul className="space-y-4">
                {prayers.map((prayer) => {
                  const busy = busyPrayerIds.includes(prayer.id);
                  return (
                    <li key={prayer.id} className="rounded-3xl border border-sand bg-white p-6 shadow-soft">
                      <div className="flex flex-wrap items-start justify-between gap-3">
